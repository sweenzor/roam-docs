...
for (const g of graphs) {
  ...
  const out = [
    ...
    ...(g.cfg.apiInventory
      ? [
          `> Two appendices at the end are generated by this mirror and are NOT part of the`,
          `> graph: a live-introspected inventory of window.roamAlphaAPI v${API_VERSION}, and the full`,
  ...
  if (g.cfg.apiInventory) {
    // Longer fence if the .d.ts ever contains ``` itself.
    const fence = '`'.repeat(Math.max(3, (dts.match(/`+/g) || ['']).sort((a, z) => z.length - a.length)[0].length + 1));
    out.push(
      '---',
      '',
