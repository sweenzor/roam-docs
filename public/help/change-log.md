...
- [[September 11th, 2026]]
  - [[Roam Depot Extensions]]
    - **RoamPrompt**
      ...
      - **What makes RoamPrompt different?**
        ...
        - For source-grounded PDFs, RoamPrompt separates:
          ...
          - what the authors interpret;
          - what the reader synthesises; and
      ...
      - **Documentation**
        - [Getting Started](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/docs/GETTING_STARTED.md)
        - [User Guide](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/docs/USER_GUIDE.md)
        - [Privacy](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/docs/PRIVACY.md)
        - [Frequently Asked Questions](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/docs/FAQ.md)
        - [Security](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/SECURITY.md)
        - [Changelog](https://github.com/Puli-AI/RoamPrompt-Public/blob/main/CHANGELOG.md)
      - **Requirements**
    ...
    - **Favicon Manager for Roam Research**
      - A small Roam Research extension that displays the favicon (website icon) next to every external link — with a settings panel, so you never have to touch code.
      - > Favicon Manager is a fork of [roam-show-favicon](https://github.com/paulovieira/roam-show-favicon) by Paul Vieira (paulovieira). All credit for the original idea and its implementation belongs to him.
        > The original extension worked exactly as well as this one, but it could only be configured by editing its source. Making those options user-configurable was on the original author's own TODO list — this fork does that, and nothing else.
      - **What it does**
        - Shows a small website icon next to every external link in Roam (main area and right sidebar), exactly like the original extension.
        - Adds a settings panel (Roam Settings → Extension Settings → Favicon Manager):
      - **Custom icons**
        - In the settings panel, under Custom icons, write a mapping as `domain=image-url`:
        - ```plain text
          github.com=https://github.com/favicon.ico```
        - To give several websites their own icon, separate the mappings with a semicolon:
        - ```plain text
          ...
          - The domain must match the link exactly — a mapping for `github.com` does not apply to `gist.github.com`.
          - The part after `=` is any image URL.
      - **Install (Roam Depot)**
        - Open Roam Research → Settings → Roam Depot.
        - Find Favicon Manager and enable it.
        - Open its Settings tab to tune things.
      - **For developers**
        - `extension.js` is the only required file. It `export default`s `{ onload, onunload }`.
        - Configuration lives in `extensionAPI.settings`; there is no build step and no runtime dependency.
        - This is a fork of [paulovieira/roam-show-favicon](https://github.com/paulovieira/roam-show-favicon). Please keep the attribution intact if you fork further.
- [[September 9th, 2026]]
  ...
  - [[Roam Depot Extensions]]
    - **Quick Switcher**
      ...
      - **Features**
        ...
        - Open saved pages and blocks from a Roam-style searchable dialog.
        - Switch between `Open` and `Manage` modes inside the Quick Switcher dialog.
        - Search Roam pages and blocks in `Manage` mode to add new saved entries.
        - Review, delete, and alias saved entries from `Manage` mode.
        - Search saved entries by alias, original title, or URL.
        - Open saved entries in the main window, or open them in the right sidebar with `Shift+Enter`, `Shift`-click, or the `Open in sidebar` footer action.
        - Optionally expose saved entries as command palette commands with a custom `QS: ` prefix. Aliased entries include both names, like `Alias (Original title)`.
      - **Settings**
        - `Open Dialog`: opens the Quick Switcher in `Open` mode.
        - `Manage Saved Entries`: opens the Quick Switcher in `Manage` mode.
        - `Add Saved Entries To Command Palette`: creates one command palette command per saved entry.
        - `Command Palette Prefix`: customizes the prefix used for saved-entry command palette commands.
    - **Custom Dark Mode**
    ...
    - **Roam Places**
      - Places helps you find and save locations while you write in Roam.
      - ![Roam Places search results with a map preview](https://raw.githubusercontent.com/MaskyS/roam-places/main/assets/roam-places-picker.webp)[🔗](https://raw.githubusercontent.com/MaskyS/roam-places/main/assets/roam-places-picker.webp)
      - Want to see several saved locations together? [Roam Map](https://github.com/MaskyS/roam-map) turns location pages written by Places into persistent maps sourced from an outline, a native Roam query, or Datalog. Places handles location capture; Roam Map handles aggregation and map rendering.
      - **Add a place**
        - Type `/` and choose Place.
        - Keep typing in the same block.
        - Use the arrow keys and Enter, or click a result.
        - Places shows matching location pages from your graph before online results. Graph results lead with their exact Roam page title. Their second line may show an address, coordinates, location status, or an online place name when the Geocoder ID matches. Choosing one inserts its page reference without changing the page.
        - The footer previews where Enter will save the highlighted result, for example `Save to Artisan Coffee ↵`. That preview changes as you move through results. When the highlighted result has coordinates, press Control–M to show or hide a map preview. The arrow keys continue moving through results and update the preview. It is only for checking the location; use Pick on map when you want to choose or adjust a point.
        - For an online result or exact pin, press Command–Enter on macOS or Control–Enter elsewhere to choose a different existing page or new page title. Choosing a page there saves immediately. A graph result always inserts its existing page. Press Escape to return to the place results without saving.
        - Escape from the main picker cancels it and removes only the text entered for that search.
      - **Page names**
        - New places use ordinary page titles by default:
        - ```plain text
          [[San Francisco City Hall]]```
        - A place does not have to live under `Places`. It may fit an existing structure such as `[[Cafe]]/Artisan Coffee` or `[[People]]/Jane Smith`, and the same page can still carry location details.
        - If you nearly always use one parent page, set Default parent page. Entering `Cafe`, for example, suggests `[[Cafe]]/Place name` for new results. The setting only changes suggestions; it does not classify places or rename existing pages. The Change page action can choose a different target for any online result or exact pin.
        - Choosing an existing page through Change page is explicit permission to add the selected place's details when the page has no `Coordinates`, `Geometry`, or `Geocoder ID`. If any of those attributes already exists, Places does not compare or overwrite it and asks you to choose another page.
      - **Map**
        - Pick on map stays at the end of the results. The map has the same place search, and you can click or drag its marker. Use this point automatically looks up nearby names with the selected place-search service. Choose one of those results or use the exact pin. A failed map or nearby-place lookup does not discard a coordinate you already chose.
        - Current location is hidden because Roam Desktop does not provide the required browser geolocation surface. Progress is tracked in [issue 1](https://github.com/MaskyS/roam-places/issues/1). Places does not substitute an estimated IP location.
      - **Place search service**
        - Photon works immediately and is the default. Geoapify is an optional alternative: choose it under Search places with, then paste your Geoapify API key. The key setting is shown only while Geoapify is selected.
        - Graph search starts with the first character. Online search starts after three characters and waits briefly for typing to pause. Each source returns at most 12 results, and stale online requests are cancelled as the query changes.
        - Under Search home, choose Pick on map to rank nearby online results first. Click the map or drag its pin, then choose Use this point. This is a soft preference, not a geographic filter, so a sufficiently specific search can still find places anywhere. A city or regional center is usually enough; avoid choosing your exact home address. Clear the setting to return to worldwide ranking.
        - Search home is stored with this Roam graph and syncs with its extension settings. People who share the graph can see inherited extension settings. Its coordinates are sent to Photon or Geoapify with each online place search as a ranking bias.
        - The Geoapify API key is also stored with the graph. People who share the graph can see an inherited key, and every Geoapify search or reverse lookup sends it to Geoapify as the `apiKey` request parameter. Use a key with appropriate usage limits.
        - Searching your graph and saving an exact map point remain available if the selected service cannot be used.
      - **What Places saves**
        - Places writes ordinary, readable Roam attributes under a direct metadata block when it creates a page or explicitly enriches an existing page. `Coordinates` is always written; the other children appear when applicable:
        - ```plain text
          roam/meta::
            Coordinates:: geo:<latitude>,<longitude>[;u=<metres>]
            Address:: <address>
            Geocoder ID:: <search-result identity>
            Geocoded at:: [[<date>]]
            Geocoder attribution:: <attribution>```
        - `roam/meta::` is Roam's metadata container. Keep that block text unchanged so Roam continues to treat the nested attributes as details about the page. Places creates it collapsed when needed and reuses an existing direct child without changing its open or collapsed state. Inserting a graph result writes nothing to that page.
        - `Coordinates` contains a plain WGS84 `geo:` URI, with latitude first and longitude second. It is not a Markdown link. The optional standard `u` parameter records uncertainty in metres. A raw exact pin may contain only the `Coordinates` child. A provider result normally adds an address, Geocoder ID, date, and attribution; a reverse-geocoded exact pin has no fabricated Geocoder ID.
        - Places does not add a `Type` attribute or force a page hierarchy. Existing pages with a top-level or metadata-contained `Coordinates`, `Geometry`, or `Geocoder ID` attribute are treated as places regardless of their title. A valid `Coordinates` geo URI is required for a point or map preview. `Geometry` marks a page as a place but is not parsed or rendered on the map.
      - **Network use**
        - After three characters, place-search text is sent to Photon, or to Geoapify when you select it. When Search home is set, its coordinates are sent with that text as a ranking bias.
        - Opening a map loads its style, sprites, fonts, and tiles from OpenFreeMap and the services referenced by that style.
        - Choosing Use this point sends the selected coordinate to Photon or Geoapify for an automatic reverse lookup of nearby names.
        - Geoapify requests include the graph-stored API key as an `apiKey` parameter.
        - Searching pages already in your graph does not use an external service.
        - Photon and map data include OpenStreetMap contributors. Places keeps the required attribution on pages it creates or enriches.
    - **Roam Map**
      ...
      - Install [Roam Places](https://github.com/MaskyS/roam-places) alongside Roam Map. Roam Map renders location data; Roam Places is the recommended way to create it. Its `/Place` command searches your graph and online place services, then saves the coordinates Roam Map reads — no attribute editing by hand. Everything below works without it, but every step is easier with it.
      - ![A Roam Map showing five places, its toolbar, a marker card, and the source outline](https://raw.githubusercontent.com/MaskyS/roam-map/main/assets/roam-map-basic-example.webp)[🔗](https://raw.githubusercontent.com/MaskyS/roam-map/main/assets/roam-map-basic-example.webp)
      - **Create your first map**
        - Give each place page a `Coordinates` attribute. The quickest way is the Roam Places `/Place` command, which saves this while you write. To add one by hand instead:
        - ```plain text
          roam/meta::
            Coordinates:: geo:-20.1609,57.5012```
        - Type `/map` and choose Map (Roam Map), or type `{{map}}` in a block.
      ...
      - **Choose what appears**
        ...
        - See the [tested query and Datalog examples](https://github.com/MaskyS/roam-map/blob/main/examples.md#native-query-and-datalog-source-maps) for copyable patterns, including relationship-aware maps.
      - **Use the map**
        ...
        - Use the basemap menu to preview another style. Add a child such as `map/basemap:: OpenFreeMap Dark` when the choice should be saved with the map.
        - Hover near the bottom-right corner and drag the resize grip. Roam Map saves the result in one readable `map/size` child; Reset map size restores the responsive default.
      ...
      - **Location data**
        - `Coordinates` uses a plain two-dimensional WGS84 `geo:` URI with latitude first and longitude second. An optional `u` parameter records uncertainty in metres. Roam Map can also render a GeoJSON Point from `Geometry`; lines, polygons, and other non-point geometry are reported rather than drawn as markers.
        - `Address` and `Geocoder ID` add useful place details. Other scalar attributes can drive advanced MapLibre styling. Roam Map reads both current Roam attributes and compatibility `Attribute:: value` blocks, including attributes beneath an exact `roam/meta::` child. Rendering never rewrites a source block or location page.
      - **Basemaps**
      ...
      - **Current limits**
        ...
        - Dynamic membership comes from native queries or fenced Datalog, not search components, saved `:q` components, or `{{map: ...}}` arguments.
        - Roam Map does not geocode, classify, or rename sources. Use Roam Places or edit the location page directly.
    - **Roam Grid**
      ...
      - **Quick start**
        - Put the cursor in any native `{{[[table]]}}` block.
        - Run Roam Grid: Enhance this table from the command palette.
        ...
        - {{[[table]]}}
          - What
            - Where
              - When
          - Cell contents
            - The original Roam blocks, unchanged in structure
              - Whenever you edit a cell
          - Table layout (column widths, row heights, merges, alignment, header visibility, chart specs)
            - Blocks on the `[[roam/grid/metadata]]` page, one per table
              - The page is created on the first table you enhance, never on install
          - Saved templates
            - A name block per template on the `[[roam/grid/templates]]` page, with the template itself as a real `{{[[table]]}}` table under it
              - Only when you run Roam Grid: Save current grid as template
          - Legacy template backups
            - `roam-grid/template-backup::` blocks on the `[[roam/grid/metadata]]` page
              - Only if a one-time migration rewrites a pre-0.12 JSON template into an editable table
          - Comment threads
            - A collapsed `[[roam/comments]]` block on the commented cell's own page, exactly the structure Roam's own comments use
              - Only when you add a comment to a cell
          - Large-grid data (experimental, off by default)
            - JSON files in Roam's file storage, pointed at by a manifest block under the `{{[[roam/grid]]}}` block
              - Only for large grids you explicitly create
          - Large-grid reference mirror (experimental, off by default)
            - Collapsed blocks under that same `{{[[roam/grid]]}}` block, listing the distinct `[[page]]`, `#tag` and `((block))` references that grid's cells contain
              - Only while Large grids — Mirror large-grid references into Roam is on
          - Live range views
            - The `{{roam-grid-range: ((uid)) A1:D5}}` text you type into a block
              - Only when you paste a range reference
    ...
    - **Publora for Roam Research**
      - Turn a block or a page into a social post without leaving Roam. Pick the accounts you connected in [Publora](https://publora.com/?utm_source=roam&utm_medium=extension), write or edit the text, then publish now, save a draft, or schedule it.
      ...
      - **Commands**
        - Open the Command Palette and run one of:
          - Publora: send this block — sends the block you are editing.
          - Publora: send this page — sends the page you have open, blocks joined in order.
        - A window opens with your connected accounts, the text, and the choice between publishing now, saving a draft and scheduling for a time you pick.
      - **Setup**
        - Create a Publora account and connect at least one social account. The free plan is 15 posts a month and three accounts, no card needed.
        - Open Settings → Publora in Roam and paste an API key from [Settings → API keys](https://app.publora.com/dashboard/api?utm_source=roam&utm_medium=extension).
      - **Things worth knowing**
        ...
        - Your key stays in Roam's extension settings and is sent only to `https://api.publora.com` in a request header. The extension talks to no other host, reads nothing else from your graph, and sends only the block or page you chose, only when you run the command.
    - **Nautilus Log**
      - > Give every minute a job.
      - A transparent visual day planner for Roam Research. Nautilus Log places fixed events and flexible tasks on one spiral timeline, so you can see what fits before the day gets away from you.
      - English · [简体中文](https://github.com/404KSG/roam-nautilus-log/blob/main/README.zh-CN.md) · [User guide](https://github.com/404KSG/roam-nautilus-log/blob/main/docs/guide.md)
      - ![Nautilus Log visual day planner with Timing, Plan, Review, and right-sidebar views](https://raw.githubusercontent.com/404KSG/roam-nautilus-log/main/docs/assets/nautilus-log-overview.png)[🔗](https://raw.githubusercontent.com/404KSG/roam-nautilus-log/main/docs/assets/nautilus-log-overview.png)
      - **See the whole day, not just a list**
        - A to-do list tells you what matters, but not whether it fits. Nautilus Log turns today's Daily Note into a living time plan:
          - See capacity before committing. Planned demand, remaining time, overload, and work that will not fit stay visible.
          - Keep structure without becoming rigid. Events stay fixed while unfinished tasks flow forward around them in Roam block order.
          - Connect estimates with reality. Plan with low-friction estimates, then optionally compare Planned with `LOGBOOK::` / `CLOCK:` Actual time.
          - Stay inside Roam. Focus, task switching, timing, completion, and daily Review remain connected to ordinary Roam blocks.
      - **One workflow, three layers**
        - Block commitments. Write meetings, meals, and routines as fixed ranges.
        - Timebox intentions. Order flexible TODOs and give each one an estimate.
        - Review reality. Optionally record Actual time and improve future estimates.
        - The scheduler is deterministic: fixed events claim their ranges first, then whole tasks fill suitable gaps from the current moment. Unfinished work moves forward without changing priority. Anything that still cannot fit appears in Today won't fit instead of disappearing. No AI estimates and no black-box scheduling.
      - **Quick start**
        - During Depot review, install the [Roam Depot preview PR](https://github.com/Roam-Research/roam-depot/pull/1428) with ShortHand `404KSG+roam-nautilus-log+1428`.
        - On today's Daily Note, type `;;` and choose Nautilus Log.
        - Add events and TODOs as direct children of the component.
        - Order the tasks, add rough durations, and adjust until the day fits.
        - ```plain text
          05:00-06:00 Morning routine
          {{[[TODO]]}} Write project brief 45m
          {{[[TODO]]}} Review notes 30m
          11:45-12:30 Lunch```
        - Durations support `30m`, `30min`, `1h`, and `1h30m`. Untimed tasks use the configured default duration.
      - **Execution Layer—when you want it**
        - The optional Execution Layer defaults to off, so estimate-only planning stays light. Enable it in Settings to add a compact execution panel:
        ...
        - Only one CLOCK runs at a time. Clock In can place the active task at the top of Roam's right sidebar, keeping focus and task switching close to native Shift+Click.
        - See the [full user guide](https://github.com/404KSG/roam-nautilus-log/blob/main/docs/guide.md) for syntax, metrics, settings, commands, Actual-history rules, and safety boundaries.
      - **Credits and inspiration**
        - [Nautilus](https://github.com/tombarys/roam-depot-nautilus) by Tomáš Barys—the original transparent spiral-planning concept.
        - [Nautilus Enhanced](https://github.com/hopeserena/nautilus-enhanced)—the enhanced fork from which this project developed.
        - [Roam Logbook](https://github.com/forrestchang/roam-logbook)—the inspiration for compatible CLOCK tracking and focused task execution.
        - The time-allocation philosophy is inspired by the [YNAB Method](https://www.ynab.com/the-four-rules/). Nautilus Log is not affiliated with YNAB. Released under the original MIT license.
- [[August 11th, 2026]]
  - [[Roam Depot Extensions]]
    - **Contribution Graph**
      ...
      - ![Contribution Graph displayed inside Roam Research](https://github.com/404KSG/contribution-graph/raw/3686d356e14c8ee0924ceb7b7c000040730e25fc/docs/images/contribution-graph-dialog.png)[🔗](https://github.com/404KSG/contribution-graph/blob/3686d356e14c8ee0924ceb7b7c000040730e25fc/docs/images/contribution-graph-dialog.png)
      - **Highlights**
    ...
    - **Roam Remove Tag**
      - Remove page tags from one or many Roam Research blocks without leaving the editor.
      - ![Roam Remove Tag demo](https://github.com/404KSG/roam-remove-tag/raw/ee3ee5ea1a0c61ada0863a521bc1763092ec3373/assets/roam-remove-tag-demo-v2.gif)[🔗](https://github.com/404KSG/roam-remove-tag/blob/ee3ee5ea1a0c61ada0863a521bc1763092ec3373/assets/roam-remove-tag-demo-v2.gif)
      - **Features**
        - Adds Remove Block's Page Tag and Remove All Tags & Aliases to both the single-block and multi-select context menus.
        - Handles `#tag`, `#[[Page Title]]`, Unicode tags such as `#café`, and `[*](((uid)))` star aliases.
        - Preserves URL fragments, arbitrary backtick code spans, fenced code, unrelated tags, children, and existing whitespace.
        - Supports current and legacy Roam context-menu UID shapes.
        - Uses Roam's native Blueprint notifications and reports updated or failed blocks.
        - Safely rolls back partial menu registration and prevents duplicate Depot/`roam/js` instances.
        - Only explicitly selected blocks are edited. Selecting a parent does not recursively modify unselected descendants.
    - **Roamoji**
      - Insert emoji inline while you type.
      - ```plain text
        :sli  →  🙂 :slightly_smiling_face:
        :tada →  🎉 :tada:
        :hrt  →  ❤️ :heart:```
      - **Using it**
        - Type your trigger (`:` by default) at the start of a word, followed by at least two characters. A list appears at the cursor.
        - {{[[table]]}}
          - Key
            - Action
          - `↑` `↓`
          ...
          - `Enter` or `Tab`
            - Insert the highlighted emoji
          - `Esc`
            - Close the list
          - `Space`
            - Close the list and keep typing
        - If you already know the shortcode, type it in full and close it with a second `:` — `:tada:` inserts 🎉 immediately without opening the list.
        - Searching matches shortcodes first, then keywords, then descriptions. When nothing matches exactly, a looser search takes over, so `:hrt` still finds ❤️.
      - **Settings**
        - Found under Roam Depot → Roamoji.
        - {{[[table]]}}
          - Setting
            - Default
              - What it does
          - Trigger
            - `:`
              - The characters that open the picker. One or two symbols; letters and digits are rejected.
          - Minimum characters
            - `2`
              - How much you type after the trigger before the list appears.
          - Results shown
            - `8`
              - How many emoji are listed at once.
          - Fuzzy fallback
            - on
              - Loose matching when nothing matches exactly.
    - **Drag References for Roam Research**
      - Turn Roam's core block-reference workflow into a fast, writing-friendly drag gesture.
      - **Demo**
        - ![Drag References demo](https://github.com/404KSG/roam-drag-references/raw/52aa2e04b5e7bf9b9682bbf668c175e4a8cd34ee/assets/demo.gif)[🔗](https://github.com/404KSG/roam-drag-references/blob/52aa2e04b5e7bf9b9682bbf668c175e4a8cd34ee/assets/demo.mp4)
        - Click the preview for the full-quality video.
      - **Shortcuts**
        - {{[[table]]}}
          - Gesture
            - Result
          - Drag
            - Roam's native block move
          - Option + Drag
            - Roam's native `((blockUid))` reference drag
          - Command + Drag
            - `[*](((blockUid)))`
          - Control + Drag
            - `Original block text [*](((blockUid)))`
          - Shift + Drag
            - `{{embed-path: ((blockUid))}}`
      - **How It Works**
        - Hold a modifier when you start dragging a bullet, then drop before, after, or to the right of another block to create a child. The source stays untouched, and every successful reference is copied to the clipboard.
        - Plain drag and Option-drag remain native to Roam. Query results and linked-reference renderings are excluded because their hierarchy can be ambiguous.
      - **Why It Helps**
        - Keeps citations and source context inside the writing flow.
        - Makes Roam's core block references and outliner structure faster to use.
        - Keeps graph data local with no analytics or external services.
    - **QuickCuts**
      - Jump between your left-sidebar Shortcuts with number hotkeys.
        - `ctrl+1` … `ctrl+9` — open the Nth Shortcut in the main window
        - `ctrl+shift+1` … `ctrl+shift+9` — open the Nth Shortcut in the right sidebar
      - Shortcuts are matched by their order in the left sidebar and resolved on every keypress, so reordering, starring, or unstarring pages takes effect immediately. Pressing a number beyond your current Shortcuts count does nothing.
      - All commands are also available in the Command Palette, just search for `QuickCuts`.
      - **Settings**
        - Hotkey Modifier (default: `ctrl`) — one switch re-generates the default hotkeys of all 18 commands. Options: `ctrl`, `alt`, `cmd`.
        - You can also rebind any command individually in Roam's native Settings → Hotkeys panel.
        - Important
        - Custom hotkeys take precedence. If you rebind an individual command in Settings → Hotkeys, that binding wins over the unified modifier, this is Roam's native behavior and extensions cannot override it. To make the command follow the unified modifier again, click "restore default" on it in the Hotkeys panel.
      - **Compatibility**
        - Works in both Roam Desktop and the browser. `ctrl+number` is not captured by browsers. `cmd+number` may conflict with browser tab switching, so the `cmd` modifier is best used in Roam Desktop.
- [[July 29th, 2026]]
