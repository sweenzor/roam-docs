...
- To choose your own embedding model and provider, select **Custom — Ollama / OpenAI-compatible endpoint**.
- Your graph contents go to the endpoint you configure rather than to Voyage AI: a hosted API you have a key for, or a local server on your own machine (Ollama, LM Studio). This is also the only option for offline graphs, since the Default option runs through Roam's servers, which offline graphs never talk to.
- How they get there depends on the endpoint. A server on your own machine or local network is called straight from your browser, with no Roam server involved. Any other endpoint is relayed through Roam's CORS proxy, which means the text being indexed and your API key pass through Roam's servers on the way. The proxy forwards each request to your endpoint and hands back the reply without storing the text or the key.
...
- The form fields:
  ...
  - **API key**: only if your endpoint needs one. Keys are stored **on this device only** and never synced; collaborators enter their own key on their own device. For a remote endpoint the key travels with each request through Roam's proxy, which passes it on without storing it.
  ...
  - **Rerank URL**: optional, but we recommend setting it up. Reranking is a second pass where a more accurate (but slower) model re-sorts the top results before you see them. Without it, Roam orders results by blending the keyword and semantic rankings by position, which is a rougher guide to what you were actually after. Point this at a Cohere-style `/rerank` endpoint, for example `https://api.cohere.com/v2/rerank`. Leave blank to skip.
...
- **Custom setup: a remote API**
  ...
  - Requests to a remote endpoint are routed through Roam's CORS proxy, because browsers block direct calls to APIs that don't send CORS headers. Your text and API key pass through Roam's servers on the way to the provider, and Roam forwards them without keeping a copy.
    - "Local" here means `localhost`, an address on your own network (like `192.168.1.20`), a `.local` or `.lan` name, or a Tailscale address. Anything else counts as remote, including a public domain name that happens to point at a server you run yourself.
- **Custom setup: Ollama (local and free)**
  ...
  - **Reranking with Ollama:** Ollama doesn't offer a rerank endpoint, so the rerank fields stay blank unless you set one up separately. Semantic search works without it, but reranking is worth adding. To keep everything on your own machine, run a local reranker (see **Custom setup: a local reranker (llama.cpp)**). A hosted service like Cohere works too, though it sends your search queries and the blocks they match through Roam's proxy.
- **Custom setup: LM Studio (local and free)**
  ...
  - **Step 3:** Fill in Roam's settings form
    ...
    - API key and Dimensions: leave blank
    - Click **Save**.
  - **Reranking with LM Studio:** LM Studio doesn't offer a rerank endpoint, so the rerank fields stay blank unless you set one up separately. Semantic search works without it, but reranking is worth adding. To keep everything on your own machine, run a local reranker (see **Custom setup: a local reranker (llama.cpp)**). A hosted service like Cohere works too, though it sends your search queries and the blocks they match through Roam's proxy.
